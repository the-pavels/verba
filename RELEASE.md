...
The version defaults to `[workspace.package].version` in `Cargo.toml` and the build number defaults to the current release build, `16`. Pass both explicitly when preparing a later build:

```sh
./scripts/package-release.sh 1.0.1 17
```
