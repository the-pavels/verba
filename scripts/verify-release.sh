...
[[ "$(plist_value "${info_plist}" SUAllowsAutomaticUpdates)" == "false" ]] || fail "users must not be offered silent automatic update installation"
[[ "$(plist_value "${info_plist}" SUEnableSystemProfiling)" == "false" ]] || fail "Sparkle system profiling must be disabled"
...
echo "  updates: Sparkle ${expected_sparkle_version}, signed feed, opt-in checks, no profiling or silent installation"
