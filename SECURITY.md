...
| Keychain | The OpenAI API key is stored as a generic-password item identified by service `com.example.Verba` and account `openai-api-key`. Rust and Swift expose configuration state and typed failures, never the stored value. The key is loaded only for a connection test or proofreading request. |
...
- `com.example.Verba` is a temporary bundle and Keychain service identifier. The permanent identifier must be chosen before signing; changing the Keychain service requires either a migration while the old item remains accessible or asking pre-release users to enter the key again.
...
- dSYMs and notarization/signing credentials are intentionally outside the app bundle. Reproducible packaging, signing identity, final entitlements inspection, third-party notice packaging, and artifact retention policy belong to roadmap items 40-42.
